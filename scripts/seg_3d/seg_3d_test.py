...
def parse_args():
    ...
    parser.add_argument('--pct_train', type=float, default=None,
                        help="Percentage of training data to use (can be None or a float)")
    ...
    parser.add_argument('--val_check_interval', type=float, default=0.25,
                        help="Interval at which to check validation performance")

    # Checkpointing arguments
    parser.add_argument('--checkpoint', type=str, default=None,
                        help="Path to a checkpoint file (can be None or a string)")
...
if __name__ == '__main__':
    ...
    ckpt_path = '/workdir1/cn24/program/SimLVSeg/lightning_logs/version_158/checkpoints/epoch=0-step=930.ckpt'
    trainer.test(model, dataloaders=dm.test_dataloader(), ckpt_path=ckpt_path)
# python scripts/seg_3d/seg_3d_test.py    --data_path /workdir1/cn24/data/pediatric_echo/A4C     --mean 0.12741163 0.1279413 0.12912785     --std 0.19557191 0.19562256 0.1965878     --encoder "SegMamba"    --frames 32     --period 1     --num_workers 2     --batch_size 4     --epochs 60      --val_check_interval 0.25     --seed 42
